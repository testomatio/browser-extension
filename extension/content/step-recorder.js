...
(() => {
  ...
  // Where the step happened: a frame's own hostname is all a cross-origin frame knows of itself.
  ...
  function send(entry) {
    ...
    // The one place the frame clause is appended, onto BOTH strings: a `replaces` the worker can no longer match leaves the twins behind.
    if (FRAME_CLAUSE) {
  ...
  // The pill is the top frame's, so in a frame there is nothing to draw and nothing to unbind.
  let host = null;
  let render = () => {};
  let dragTeardown = () => {};
  ...
  function installPill() {
    ...
    dragTeardown = function dragTeardown() {
  ...
  if (host) (document.body || document.documentElement).append(host);
  render();
  ...
  // The top frame only: a payment form's <title> is not the name of the page the tester navigated to.
  if (TOP) {
